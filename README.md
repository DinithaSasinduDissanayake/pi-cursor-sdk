...
# Transcript leak mitigations (on by default; upstream issue #40).
PI_CURSOR_RETRY_MARKER=0 pi --model cursor/composer-2-5
PI_CURSOR_TRANSCRIPT_TAGS=0 pi --model cursor/composer-2-5
PI_CURSOR_LEAK_GUARD=0 pi --model cursor/composer-2-5
PI_CURSOR_LIFECYCLE_PERSIST=1 pi --model cursor/composer-2-5
...
Bootstrap prompts also wrap prior message history in `<transcript>` tags by default (`PI_CURSOR_TRANSCRIPT_TAGS=1`) and prefix identical user retries after an aborted turn with a short interruption notice (`PI_CURSOR_RETRY_MARKER=1`). When Composer emits transcript-format tool-call text instead of real SDK tool calls, output is quarantined by default (`PI_CURSOR_LEAK_GUARD=1`). The guard suppresses any line that starts with `[ran tool` (only pi-cursor-sdk serialization produces that prefix at line start); quoted examples on their own line may be suppressed as an acceptable tradeoff. Mid-line mentions stay safe. Set any of these to `0` to disable that mitigation independently.

Deferred Cursor tool lifecycle progress (`Cursor shell: ...`, `Cursor MCP: ...`, shell-output previews, and `Cursor summary:` lines) still streams live in the TUI during a turn, but is not persisted into session thinking blocks by default (`PI_CURSOR_LIFECYCLE_PERSIST=0`). Composer's own `thinking-delta` output is still persisted. Set `PI_CURSOR_LIFECYCLE_PERSIST=1` to restore the old persistence behavior. Lifecycle text still lands in `PI_CURSOR_SDK_EVENT_DEBUG` artifacts when that debug mode is enabled.
